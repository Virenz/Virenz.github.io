...
class Create_newMD:
    ...
    def create(self):
        ...
        if (not os.path.exists(self.path)):
            ...
            for file in filelist:
                with open(self.path+"/"+file,'r',encoding='utf-8')as f:
                    ...
                    while switch <= 20:
                        ...
                        # 获取typora-root-url
                        if "typora-root-url: " in content:
                            tag_ = content.split("typora-root-url: ")[1].strip().replace("\n","")
                            self.root_url_list.append(tag_)
                        # else:
            ...
            all_categories.sort()
            all_root_urls = list(set(self.root_url_list))
            all_root_urls.sort()
            ...
            while True:
                ...
                if input_title != "":
                    ...
                    description = "description: " + input_title.replace("(","（").replace(")","）").replace(",","，").\
                    replace("[","【").replace("]","】").replace("/","_").replace("%","_").replace("#","_").\
                    replace("<", "《").replace(">", "》").replace("&", "_").replace("'", '"').replace("`", "_"). \
                    replace("^", "_").replace("@", "_").replace("$", "_").replace("!", "！").replace("?", "？").replace(";", "；")
                    # 结尾不能是英文冒号
            ...
            print("\n" + "===========已添加过的typora-root-url如下===========")
            print("{:^15}\t{:^10}".format("typora-root-url代号", "typora-root-url名称"))
            tags_dic = {}
            for j, chr in enumerate(all_root_urls):
                print("{:^18}\t{:^12}".format(str(j + 1) + "-", chr))
                tags_dic[str(j + 1) + "-"] = chr
            input_tag = input("\n" + "输入文章标签名(不输入默认为分类名或输入相应标签代号，注意后面的减号):").strip()
            if input_tag != "":
                if input_tag in tags_dic.keys():
                    tag = "typora-root-url: " + tags_dic[input_tag]
                else:
                    tag = "typora-root-url: " + input_tag
            else:
                if input_category.isdigit():
                    tag = "typora-root-url: " + input_category
                else:
                    tag = "typora-root-url: " + category.split("categories: ")[1]
            print("标签名为：%s" % tag.split("typora-root-url: ")[1])
